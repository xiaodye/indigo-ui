...
const defaultImage = (
  <svg className="icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" width="70" height="70">
    <path d="M102.4 896a409.6 51.2 0 1 0 819.2 0 409.6 51.2 0 1 0-819.2 0Z" fill="#dbdbdb" opacity=".1" />
    <path
...
const Empty: FC<EmptyProps> = ({ style, className, description = '暂无数据', image = defaultImage, imageStyle }) => {
  const classes = classNames('cobalt-empty', className);

  return (
    <div className={classes} style={style}>
      {image && (typeof image === 'string' ? <img src={image} alt="empty" style={imageStyle} /> : image)}

      {description &&
        (typeof description === 'string' ? (
          <div className="cobalt-empty-description">{description}</div>
        ) : (
          <div>{description}</div>
        ))}
    </div>
